...
func main() {
	removeIndicatorsRequest := *openapiclient.NewRemoveIndicatorsRequest([]string{"IndicatorIds_example"}) // RemoveIndicatorsRequest | The list of indicator IDs to remove
...
func main() {
	uploadBlobIndicatorsRequest := *openapiclient.NewUploadBlobIndicatorsRequest("FreeTAXII", time.Now(), "Format_example", "Blob_example") // UploadBlobIndicatorsRequest | Upload blob indicators request body.
...
> UploadNormalizedIndicators(ctx).NormalizedIndicator(normalizedIndicator).Execute()
...
func main() {
	normalizedIndicator := []openapiclient.NormalizedIndicator{*openapiclient.NewNormalizedIndicator("indicator--d81f86b9-975b-4c0b-875e-810c5ad45a4f", "182.158.1.1", "ipv4-addr", "FreeTAXII", time.Now(), time.Now(), int32(123), "indicator")} // []NormalizedIndicator | The list of normalized threat intel indicators to upload.

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	r, err := apiClient.ThreatIntelIngestAPI.UploadNormalizedIndicators(context.Background()).NormalizedIndicator(normalizedIndicator).Execute()
	if err != nil {
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **normalizedIndicator** | [**[]NormalizedIndicator**](NormalizedIndicator.md) | The list of normalized threat intel indicators to upload. | 
...
> UploadStixIndicators(ctx).UploadStixIndicatorsRequest(uploadStixIndicatorsRequest).Execute()
...
func main() {
	uploadStixIndicatorsRequest := *openapiclient.NewUploadStixIndicatorsRequest("FreeTAXII", time.Now(), []openapiclient.StixIndicator{*openapiclient.NewStixIndicator("indicator", "2.1", "acme:indicator-bf8bc5d5-c7e6-46b0-8d22-7500fea77196", time.Now(), time.Now(), "[ipv4-addr:value = '1.2.3.4']", "stix", time.Now())}) // UploadStixIndicatorsRequest | Upload stix indicators request body.

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	r, err := apiClient.ThreatIntelIngestAPI.UploadStixIndicators(context.Background()).UploadStixIndicatorsRequest(uploadStixIndicatorsRequest).Execute()
	if err != nil {
